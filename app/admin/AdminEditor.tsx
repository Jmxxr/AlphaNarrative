...
export function AdminEditor() {
  ...
  async function save() {
    ...
    setBusy(false); setMessage(response.ok ? "Saved. Your host may take a moment to publish the update." : (await response.json()).error || "Could not save.");
  }
  ...
  return <main className="admin-screen"><div className="admin-dashboard"><div className="admin-head"><div><p className="eyebrow">Alpha Narrative / Studio</p><h1>Case studies</h1></div><div><button className="quiet-link" onClick={() => { setProjects([...projects, {...empty}]); setSelected(projects.length); }}>+ New project</button><button className="quiet-link" onClick={async () => { await fetch("/api/admin/session", { method: "DELETE", headers: { "Content-Type": "application/json" } }); setProjects(null); }}>Sign out</button></div></div>
